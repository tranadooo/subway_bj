...
sql = open('../sql/load_and_display.sql','r').read().replace("datescope",datescope)
print(sql)
cur.execute(sql)
tmp = cur.fetchall()
...
print('--------开始写入接口文件数据-----------')
count = 1
for outdir,date_type in zip(outdirs,date_types):
    for table_code,table_name,train_size in zip(table_codes,table_names,train_sizes):
        ...
        if tmp_df.shape[0] > 0:
            tmp_df.to_csv(outdir+out_file,header=0,index=None)
            print('写入第%s个文件%s到%s''成功'%(count, out_file,outdir))
        else:
            print('没有第%s个文件%s到%s''数据'%(count, out_file,outdir))
        count += 1
print('-----写入接口文件成功-----')
...
for outdir,date_type in zip(outdirs,date_types):
    for table_name,train_size in zip(table_names,train_sizes):
        ...
        if tmp_df.shape[0] > 0:
            tmp_df.to_excel(outdir+out_file,index=None)
            print('写入第%s个文件%s到%s''成功'%(count, out_file,outdir))
        else:
            print('没有第%s个文件%s到%s''数据'%(count, out_file,outdir))
        count += 1
