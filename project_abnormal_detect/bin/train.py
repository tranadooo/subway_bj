...
if __name__=='__main__':
    ...
    for date_scope,exclude_date_scope,train_date_type,train_target,train_time_size in combinations:
        ...
        hdfs_file='hdfs:/user/station/bmnc_limcp.db/train_result_detail/train_id=%s'%task_id
    ...
    #根据训练的日期类型从数仓导出数据更新到目录
    file_date_type=",".join(train_date_types)
    os.system( "python3 -u  load_and_save.py  \'%s\'   \'%s\'   \
           " % (date_scope,file_date_type)
         )
    
    #根据次日类型选择目录生成chk并推送
    from ftp import push_file
    push_file(tomorrow_type)
