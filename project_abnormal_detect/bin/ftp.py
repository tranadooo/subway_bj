...
def push_file(tomorrow):
    ...
    if tomorrow=='1':
        ...
        print('cd ../data/output_ho')
        os.chdir('../data/output_ho')
        fl = os.listdir()
        fl = [i for i in fl if i.endswith('dat')]
        print(fl)
        print('tomorrow is holiday ---start upload holiday threshold---')
        create_chk_and_ftp(fl)
    else:
        print("tomorrow not workday/weekend/holiday, exit later")
    
if __name__=='__main__':    
    tomorrow = get_tomorrow_daytype()
    push_file(tomorrow)
