...
def get_time():  
    time_now = dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f : ')
    return time_now
...
def preprocessing(data_series_chara):
    ...
    try:
        data_series_chara["日期"] = data_series_chara["日期"].apply(lambda x:str(x).replace('-',''))
        data_series_chara['总量'] = data_series_chara['总量'].astype(int)
        data_series_chara['疫情'] = data_series_chara['疫情'].astype(float)
