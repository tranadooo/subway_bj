select t2.line_id line_id,t2.station_id station_id,t2.data_dt `日期`,t3.week `星期`,t3.disease `疫情`,
...
from(
  select t1.line_id,t1.station_id,t1.data_dt,sum(entry) sum_,count(entry) num_,concat('[',regexp_replace(
   ...
   from (
            ...
            where data_dt between '2020-04-15' and '2020-12-31'  --2020年的数据
  ...
  group by t1.line_id,t1.station_id,t1.data_dt) t2,bmnc_infer.bmnc_date_prop t3
